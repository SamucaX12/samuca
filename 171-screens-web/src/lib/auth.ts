...
import { ensureScannerUserProfile } from "./user-lookup";
...
export async function upsertUserFromDiscord(
  ...
  if (existing) {
    ...
    const updated = (await db.collection<UserDoc>("users").findOne({ discordId: user.id }))!;
    await ensureScannerUserProfile({
...
async function resolveScannerAccess(discordId: string, role: UserRole) {
  if (role === "owner" || role === "admin") {
    return { scannerPlan: "enterprise" as ScannerPlan, enterpriseId: discordId };
  }
  try {
    const db = await getScannerDb();
    const u = await db
      .collection<{ discordId: string; plan?: ScannerPlan; enterpriseId?: string | null; planExpiresAt?: Date | null }>(
        "users"
      )
      .findOne({ discordId });
    if (u?.planExpiresAt && new Date(u.planExpiresAt) < new Date()) {
      ...
      return { scannerPlan: null as ScannerPlan, enterpriseId: null };
    }
    return { scannerPlan: u?.plan ?? null, enterpriseId: u?.enterpriseId ?? null };
  } catch {
    return { scannerPlan: null as ScannerPlan, enterpriseId: null };
  }
}
...
export async function createSession(userDoc: UserDoc) {
  const scanner = await resolveScannerAccess(userDoc.discordId, userDoc.role);
...
export async function upsertUserFromGoogle(
  ...
  const db = await getDb();
  const existing = await db.collection<UserDoc>("users").findOne({ discordId: googleId });
  const baseRole = resolveOwner(googleId);
  ...
  if (existing) {
    ...
    const updated = (await db.collection<UserDoc>("users").findOne({ discordId: googleId }))!;
    await ensureScannerUserProfile({
      discordId: updated.discordId,
      email: updated.email,
      username: updated.username,
